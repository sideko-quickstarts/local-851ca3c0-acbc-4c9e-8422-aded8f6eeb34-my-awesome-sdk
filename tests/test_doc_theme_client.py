...
def test_update_200_success_all_params():
    """Tests a PUT request to the /doc_project/{doc_name}/theme endpoint.
    ...
    response = client.doc.theme.update(
        doc_name="my-project",
...
async def test_await_update_200_success_all_params():
    ...
    response = await client.doc.theme.update(
        doc_name="my-project",
        api_reference_group_variant="grouped",
        dark_active_button_bg_color="#FFFFFF",
        dark_active_button_text_color="#FFFFFF",
...
def test_get_200_success_all_params():
    """Tests a GET request to the /doc_project/{doc_name}/theme endpoint.
    ...
    response = client.doc.theme.get(doc_name="my-project")
    try:
        pydantic.TypeAdapter(models.Theme).validate_python(response)
        is_valid_response_schema = True
    except pydantic.ValidationError:
...
async def test_await_get_200_success_all_params():
    ...
    response = await client.doc.theme.get(doc_name="my-project")
    try:
        pydantic.TypeAdapter(models.Theme).validate_python(response)
        is_valid_response_schema = True
    except pydantic.ValidationError:
